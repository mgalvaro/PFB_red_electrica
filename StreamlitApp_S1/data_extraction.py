...
def extrae_balance(start_date, end_date) -> pd.DataFrame:
...
# DEFINICION DE FUNCION PARA EXTRACCION DE DEMANDA HORARIA

def extrae_demanda_horaria(dias) -> pd.DataFrame:

    hoy = datetime.now()

    horas = timedelta(days=dias)

    inicio_hour = (hoy - horas).strftime("%Y-%m-%dT%H:%M")
    fin_hour = hoy.strftime("%Y-%m-%dT%H:%M")

    df_demanda_horaria = pd.DataFrame()
    ...
    for widget in widgets["demanda"]:
        ...
        for zona in geo_limit:
            ...
            if zona != "ccaa":
                ...
                get_request_url = url + "demanda/" + widget + f"?start_date={inicio_hour}&end_date={fin_hour}&time_trunc={time_trunc[0]}&geo_trunc={geo_trunc[0]}&geo_limit={zona}&geo_ids={geo_ids[zona]}"
                #print(f"ENDPOINT: {get_request_url}")
                response = requests.get(get_request_url)
                status_response = response.status_code
                ...
                if status_response == 200:
                    ...
                    for i in response["included"]:  
                        ...
                        df_base.drop(columns="values", inplace=True)
                        df_base["extraccion"] = hoy.strftime("%Y-%m-%dT%H:%M")
                        df_base["categoria"] = response["data"]["type"]
                        df_base["zona"] = zona
                        df_valores = (pd.json_normalize((i["attributes"]["values"])))
                        df_concatenado = pd.concat([df_base,df_valores], axis=1)
                        df_demanda_horaria = pd.concat([df_demanda_horaria,df_concatenado], ignore_index=True)

    sleep(1)
    
    return df_demanda_horaria
...
# DEFINICION DE FUNCION PARA EXTRACCION DE GENERACION

def extrae_generacion(start_date, end_date) -> pd.DataFrame:

    hoy = datetime.now().strftime("%Y-%m-%dT%H:%M")
    df_generacion = pd.DataFrame()
    
    for widget in widgets["generacion"]:

        get_request_url = url + "generacion/" + widget + f"?start_date={start_date}&end_date={end_date}&time_trunc={time_trunc[1]}&systemElectric=nacional"
        #print(f"ENDPOINT: {get_request_url}")
        response = requests.get(get_request_url)
        status_response = response.status_code
        ...
        if status_response == 200:
            ...
            for i in response["included"]:   
                ...
                df_valores = (pd.json_normalize((i["attributes"]["values"])))
                df_concatenado = pd.concat([df_base,df_valores], axis=1)
                df_generacion = pd.concat([df_generacion,df_concatenado], ignore_index=True)

        for zona in geo_limit:
            
            if zona != "ccaa":
                
                get_request_url = url + "generacion/" + widget + f"?start_date={start_date}&end_date={end_date}&time_trunc={time_trunc[1]}&geo_trunc={geo_trunc[0]}&geo_limit={zona}&geo_ids={geo_ids[zona]}"
                #pri#nt(f"ENDPOINT: {get_request_url}")
                response = requests.get(get_request_url)
                status_response = response.status_code
                ...
                for ccaa in geo_ids["ccaa"]:
                    ...
                    if (geo_ids['ccaa'][ccaa] != 8742) & (geo_ids['ccaa'][ccaa] != 8743) & (geo_ids['ccaa'][ccaa] != 8744) & (geo_ids['ccaa'][ccaa] != 8745):
                        ...
                        get_request_url = url + "generacion/" + widget + f"?start_date={start_date}&end_date={end_date}&time_trunc={time_trunc[2]}&geo_trunc={geo_trunc[0]}&geo_limit=ccaa&geo_ids={geo_ids['ccaa'][ccaa]}"
                        #print(f"ENDPOINT: {get_request_url}")
                        response = requests.get(get_request_url)
                        status_response = response.status_code
